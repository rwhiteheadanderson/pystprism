    ...
    for fix in in_trajectory.fixes:
        if in_trajectory.fixes.index(fix) != in_trajectory.fixes.index(
            ...
            for disk_number in range(disk_count):
                ...
                f_i_accessible_distance = (t - t_i) * s_i_j * velocity_multiplier
                f_j_accessible_distance = (t_j - t) * s_i_j * velocity_multiplier
...
def probabilistic_space_time_prism(
    in_features,
    timestamp_field,
    disk_interval,
    cell_size,
    ...
    for fix in in_trajectory.fixes:
        if in_trajectory.fixes.index(fix) != in_trajectory.fixes.index(
            ...
            try:
                line_array = arcpy.Array(
                    [
                        ...
                        in_trajectory.fixes[in_trajectory.fixes.index(fix) + 1][
                            "geometry"
                        ].firstPoint,
                    ]
                )
                line_i_j = arcpy.Polyline(line_array, in_trajectory.spatial_reference)
                for disk_number in range(disk_count):
                    t = float(disk_number * disk_interval)
                    progress_factor = t / (t_j - disk_interval)
                    progress_distance = line_i_j.length * progress_factor
                    disk_center_point = line_i_j.positionAlongLine(progress_distance)
            ...
            for disk_number in range(disk_count):
                ...
                k_distances = Con(
                    EqualTo(k_distances, 0), (1.0 / cell_size), k_distances
                )
...
def cost_distance_based_probabilistic_space_time_prism(
    in_features,
    timestamp_field,
    disk_interval,
    cell_size,
    ...
    """Generates the CDBPSTP voxel space-time prisms from in_trajectory. This
    cdbpstp result is returned as an ordered list of arcpy raster objects. Each
    raster represents one space time disk. All timings are recorded in seconds.
    This function does not account for stationary activity time, and assumes
    the object was in-motion from departure to arrival. This function requires
    a spatial analyst license.
    """
    cost_surface_raster = arcpy.Raster(cost_surface_raster)
    # First, prepare a Trajectory object from the input dataset
    in_trajectory = Trajectory(in_features, timestamp_field)
    # Ensure the user's disk interval is smaller than the minimum time elapsed
    if disk_interval > in_trajectory.minimum_disk_interval:
    ...
    for fix in in_trajectory.fixes:
        if in_trajectory.fixes.index(fix) != in_trajectory.fixes.index(
            ...
            # Time at second anchor (time at destination fix)
            t_j = t_i + fix["elapsed_time"]
            cost_distances_from_x_i = CostDistance(x_i_layer, cost_surface_raster)
            cost_distances_from_x_j = CostDistance(x_j_layer, cost_surface_raster)
            cost_backlink = CostBackLink(x_i_layer, cost_surface_raster)
            disk_centers = []
            try:
                result = CostPathAsPolyline(
                    x_j_layer,
                    cost_distances_from_x_i,
                    cost_backlink,
                    "lcp",
                    "BEST_SINGLE",
                )
                lcp_fc = result.getOutput(0)
                arcpy.FlipLine_edit(lcp_fc)
                with arcpy.da.SearchCursor(lcp_fc, "SHAPE@") as cursor:
                    for row in cursor:
                        lcp_line = row[0]
                for disk_number in range(disk_count):
                    t = float(disk_number * disk_interval)
                    progress_factor = t / (t_j - disk_interval)
                    progress_distance = lcp_line.length * progress_factor
                    disk_center_point = lcp_line.positionAlongLine(progress_distance)
                    disk_centers.append(disk_center_point)
            except Exception as e:
                print(e)
                pass
            ...
            for disk_number in range(disk_count):
                ...
                f_i_accessible_distance = (t - t_i) * s_i_j * velocity_multiplier
                f_j_accessible_distance = (t_j - t) * s_i_j * velocity_multiplier
                f_i_cone = LessThanEqual(
                    cost_distances_from_x_i, f_i_accessible_distance
                )
                f_j_cone = LessThanEqual(
                    cost_distances_from_x_j, f_j_accessible_distance
                )
                Z_i_j_disk = BooleanAnd(f_i_cone, f_j_cone)
                result = arcpy.MakeFeatureLayer_management(
                    centers_fc,
                ...
                k_center_layer = result.getOutput(0)
                # Get a CostDistance surface radiating out from this point
                k_distances = CostDistance(k_center_layer, cost_surface_raster)
                # Fill in the center of k_distances to avoid donut effect in cdbpstp disks
                k_distances = Con(
                    EqualTo(k_distances, 0), (1.0 / cell_size), k_distances
                )
                # Use multiplication to isolate disk distance values
                ...
                if disk_idw.mean is not None:
                    ...
                    cdbpstp_rasters.append(out_tuple)
        else:
            pass
    return cdbpstp_rasters
