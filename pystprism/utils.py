...
class Trajectory(object):
    ...
    def __init__(self, in_features, timestamp_field):
        ...
        with arcpy.da.SearchCursor(
                in_features, ['OID@', 'SHAPE@', timestamp_field]) as cursor:
            for row in cursor:
                self.fixes.append({
                    'OID': row[0],
                    'geometry': row[1],
        ...
        for feature in self.fixes:
            if self.fixes.index(feature) == len(self.fixes) - 1:
                ...
                feature['elapsed_time'] = (
                    ...
                    feature['timestamp']).total_seconds()
                # velocity is the distance/time of travel from A to B
                feature['velocity'] = \
                    feature['distance'] / feature['elapsed_time']
        self.duration = \
            (self.fixes[-1]['timestamp'] -
                self.fixes[0]['timestamp']).total_seconds()
        self.count = len(self.fixes)
        self.minimum_disk_interval = min(
            fix['elapsed_time'] for fix in self.fixes if \
                fix['elapsed_time'] != 0)
        return
    ...
    def export_trajectory(self, output_workspace, output_name):
        ...
        arcpy.AddField_management(output_workspace + "\\" + output_name,
                                  "id", "LONG")
        arcpy.AddField_management(output_workspace + "\\" + output_name,
                                  "timestamp", "DATE")
        for item in ["distance", "elapsed_time", "velocity"]:
            arcpy.AddField_management(output_workspace + "\\" + output_name,
                                      item, "DOUBLE")
        with arcpy.da.InsertCursor(output_workspace + "\\" + output_name, [
                "id", "SHAPE@", "timestamp", "distance", "elapsed_time",
                "velocity"
        ]) as cursor:
            for item in self.fixes:
                row = [
                    item["OID"], item["geometry"], item["timestamp"],
                    item["distance"], item["elapsed_time"], item["velocity"]
                ]
